...
not currently supported. For ERR and linear-exponential models,
subgroup-specific modifier coding can be more numerically robust because
default bounds are applied directly to each subgroup dose-effect
parameter. See [Effect
modification](https://ameras.sanderroberti.com/articles/effectmodification.md)
for more detail. When `deg`, `modifier`, and `model` are not supplied,
the defaults are `deg=1`, no effect modifiers, and `model="ERR"`.

Additional right-hand-side covariates are expanded using
[`model.matrix()`](https://rdrr.io/r/stats/model.matrix.html), so
standard formula features such as factor contrasts, interactions,
...
`I(age^2)`, `splines::ns(age, df = 3)`, or `splines::bs(age, df = 4)`.
These terms must preserve one value per row of the input data; terms
that drop rows, such as `stats::na.omit(X)`, are not supported. When
`keep.data = FALSE`, later calls that use supplied data apply the same
expanded covariate columns as the original fit.

### Example data

The included example dataset contains 3,000 individuals with 10 exposure
realizations `V1`-`V10`, binary covariates `X1` and `X2` and effect
...
#> Warning in assemble_fma_result(FMAfits = FMAfits, dosevars = dosevars, parnames
#> = parnames, : FMA is based on only 4 realizations.
#> |-------------|-------------|-------------|-------------|
#> |-------------------------------------------------------|
#> |-------------|-------------|-------------|-------------|
#> |-------------------------------------------------------|
...
#>  $ num.rows          : int 3000
#>  $ num.rows.original : int 3000
#>  $ na.action         : NULL
#>  $ num.realizations  : int 10
...
#>   .. .. ..- attr(*, ".Environment")=<environment: base> 
#>   ..$ X_design_info:List of 3
#>   .. ..$ terms    :Classes 'terms', 'formula'  language ~X1 + X2
#>   .. .. .. ..- attr(*, "variables")= language list(X1, X2)
#>   .. .. .. ..- attr(*, "factors")= int [1:2, 1:2] 1 0 0 1
...
#>   .. .. .. ..- attr(*, "response")= int 0
#>   .. .. .. ..- attr(*, ".Environment")=<environment: 0x558723ab9310> 
#>   .. .. .. ..- attr(*, "predvars")= language list(X1, X2)
#>   .. .. .. ..- attr(*, "dataClasses")= Named chr [1:2] "numeric" "numeric"
#>   .. .. .. .. ..- attr(*, "names")= chr [1:2] "X1" "X2"
#>   .. ..$ contrasts: NULL
#>   .. ..$ xlevels  : Named list()
#>   ..$ X_names      : chr [1:2] "X1" "X2"
#>   ..$ X            : int [1:2] 9 10
#>   ..$ na_action_fun:function (object, ...)  
#>   ..$ offset       : NULL
#>   ..$ entry        : NULL
#>   ..$ exit         : NULL
#>   ..$ status       : NULL
...
#>   .. ..$ gradient                   : num [1:5] -0.00108 -0.01903 0.00946 0.01364 -0.01019
#>   .. ..$ gradient.max.abs           : num 0.019
#>   .. ..$ gradient.rms               : num 0.0122
#>   .. ..$ gradient.rms.scaled        : num 2.67e-06
#>   .. ..$ newton.decrement           : num 0.000943
#>   .. ..$ newton.improvement         : num 4.44e-07
#>   .. ..$ newton.improvement.relative: num 9.73e-11
#>   .. ..$ newton.step.max.abs        : num 3.02e-05
#>   .. ..$ convergence                : int 0
...
#>   .. ..$ gradient                   : num [1:5] 1.13e-04 5.67e-05 -4.82e-05 2.03e-04 1.89e-03
#>   .. ..$ gradient.max.abs           : num 0.00189
#>   .. ..$ gradient.rms               : num 0.000852
#>   .. ..$ gradient.rms.scaled        : num 1.87e-07
#>   .. ..$ newton.decrement           : num 2.48e-05
#>   .. ..$ newton.improvement         : num 3.06e-10
#>   .. ..$ newton.improvement.relative: num 6.72e-14
#>   .. ..$ newton.step.max.abs        : num 3.13e-07
#>   .. ..$ convergence                : int 0
...
#>   .. ..$ gradient                   : num [1:5] -0.000492 -0.009906 0.002233 -0.032603 0.09574
#>   .. ..$ gradient.max.abs           : num 0.0957
#>   .. ..$ gradient.rms               : num 0.0455
#>   .. ..$ gradient.rms.scaled        : num 9.78e-06
#>   .. ..$ newton.decrement           : num 0.00146
#>   .. ..$ newton.improvement         : num 1.06e-06
#>   .. ..$ newton.improvement.relative: num 2.29e-10
#>   .. ..$ newton.step.max.abs        : num 1.99e-05
#>   .. ..$ convergence                : int 0
...
#> 
#> $optim$gradient
#> [1] -0.001076888 -0.019027042  0.009460654  0.013635462 -0.010187664
#> 
#> $optim$gradient.max.abs
#> [1] 0.01902704
#> 
#> $optim$gradient.rms
#> [1] 0.01218529
#> 
#> $optim$gradient.rms.scaled
#> [1] 2.670265e-06
#> 
#> $optim$newton.decrement
#> [1] 0.0009425686
#> 
#> $optim$newton.improvement
#> [1] 4.442178e-07
#> 
#> $optim$newton.improvement.relative
#> [1] 9.73452e-11
#> 
#> $optim$newton.step.max.abs
#> [1] 3.01561e-05
#> 
#> $optim$convergence
#> [1] 0
...
#> Rows: 3000
#> 
#> Total CPU runtime: 500.5 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method     Fit  CI   Total
#>      RC   0.390 0.0   0.390
#>     ERC 414.249 0.0 414.249
#>    MCML   0.752 0.0   0.752
#>     FMA   1.840 0.0   1.840
#>     BMA  83.296 0.0  83.296
#> 
...
#> Rows: 3000
#> 
#> Total CPU runtime: 347.3 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method     Fit  CI   Total
#>      RC   0.389 0.0   0.389
#>     ERC 278.723 0.0 278.723
#>    MCML   1.387 0.0   1.387
#>     FMA   3.180 0.0   3.180
#>     BMA  63.613 0.0  63.613
#> 
#> Summary of coefficients by method:
#> 
#>  Method         Term Estimate      SE Rhat   n.eff
...
#> Rows: 3000
#> 
#> Total CPU runtime: 270.1 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method     Fit  CI   Total
#>      RC   0.188 0.0   0.188
#>     ERC 209.036 0.0 209.036
#>    MCML   0.912 0.0   0.912
#>     FMA   1.840 0.0   1.840
#>     BMA  58.160 0.0  58.160
#> 
#> Summary of coefficients by method:
#> 
#>  Method        Term Estimate      SE Rhat  n.eff
...
#> Rows: 3000
#> 
#> Total CPU runtime: 77.4 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method    Fit  CI  Total
#>      RC  0.353 0.0  0.353
#>     ERC  3.288 0.0  3.288
#>    MCML  1.605 0.0  1.605
#>     FMA  4.080 0.0  4.080
#>     BMA 68.086 0.0 68.086
#> 
#> Summary of coefficients by method:
#> 
#>  Method         Term Estimate       SE Rhat  n.eff
...
#> Rows: 3000
#> 
#> Total CPU runtime: 68.7 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method    Fit  CI  Total
#>      RC  0.210 0.0  0.210
#>     ERC  2.096 0.0  2.096
#>    MCML  0.888 0.0  0.888
#>     FMA  2.100 0.0  2.100
#>     BMA 63.422 0.0 63.422
#> 
#> Summary of coefficients by method:
#> 
#>  Method        Term Estimate      SE Rhat  n.eff
...
among cases. The number of such intervals can be specified through the
...
fit.ameras.prophaz <- ameras(Surv(time, status)~
                             ...
                             methods=c("RC", "ERC", "MCML", "FMA", "BMA"), 
                             niter.BMA = 5000, nburnin.BMA = 1000)
#> Warning in assemble_frequentist_fit_result(fit = fit, parnames = parnames, :
...
#> Rows: 3000
#> 
#> Total CPU runtime: 134.9 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method     Fit  CI   Total
#>      RC   0.304 0.0   0.304
#>     ERC  13.015 0.0  13.015
#>    MCML   0.723 0.0   0.723
#>     FMA   2.354 0.0   2.354
#>     BMA 118.543 0.0 118.543
#> 
#> Summary of coefficients by method:
#> 
#>  Method         Term  Estimate      SE Rhat  n.eff
...
#> Rows: 3000
#> 
#> Total CPU runtime: 124.3 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method     Fit  CI   Total
#>      RC   0.157 0.0   0.157
#>     ERC   6.346 0.0   6.346
#>    MCML   0.451 0.0   0.451
#>     FMA   1.325 0.0   1.325
#>     BMA 116.006 0.0 116.006
#> 
#> Summary of coefficients by method:
#> 
#>  Method   Term Estimate      SE Rhat  n.eff
...
#> Rows: 3000
#> 
#> Total CPU runtime: 798.2 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method     Fit  CI   Total
#>      RC   1.830 0.0   1.830
#>     ERC 499.694 0.0 499.694
#>    MCML   9.426 0.0   9.426
#>     FMA  12.399 0.0  12.399
#>     BMA 274.813 0.0 274.813
#> 
#> Summary of coefficients by method:
#> 
#>  Method             Term  Estimate      SE Rhat  n.eff
...
#> Rows: 3000
#> 
#> Total CPU runtime: 609.6 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method     Fit  CI   Total
#>      RC   1.193 0.0   1.193
#>     ERC 355.599 0.0 355.599
#>    MCML   7.271 0.0   7.271
#>     FMA  10.329 0.0  10.329
#>     BMA 235.162 0.0 235.162
#> 
#> Summary of coefficients by method:
#> 
#>  Method            Term Estimate      SE Rhat  n.eff
...
#> Rows: 3000
#> 
#> Total CPU runtime: 189.4 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method    Fit  CI  Total
#>      RC  2.724 0.0  2.724
#>     ERC 67.167 0.0 67.167
#>    MCML  9.215 0.0  9.215
#>     FMA 31.799 0.0 31.799
#>     BMA 78.510 0.0 78.510
#> 
#> Summary of coefficients by method:
#> 
#>  Method         Term Estimate      SE Rhat  n.eff
...
#> Call:
#> ameras(formula = Y.clogit ~ dose(V1:V10, deg = 1, model = "EXP") + 
#>     X1 + X2 + strata(setnr), data = data, family = "clogit", 
#>     methods = c("RC", "ERC", "MCML", "FMA", "BMA"), nburnin.BMA = 1000, 
#>     niter.BMA = 5000)
#> 
#> Rows: 3000
#> 
#> Total CPU runtime: 119.9 seconds
#> 
#> CPU runtime in seconds by method:
#> 
#>  Method    Fit  CI  Total
#>      RC  1.581 0.0  1.581
#>     ERC 30.898 0.0 30.898
#>    MCML  3.812 0.0  3.812
#>     FMA 14.509 0.0 14.509
#>     BMA 69.094 0.0 69.094
#> 
#> Summary of coefficients by method:
#> 
#>  Method Term Estimate      SE Rhat  n.eff
