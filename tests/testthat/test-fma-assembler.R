...
make_fma_summary <- function(coef, AIC, hess = diag(length(coef)), include = TRUE) {
  list(
    coef = coef,
    hess = hess,
    AIC = AIC,
...
test_that("assemble_fma_result forwards transform arguments while sampling", {
  # The marker arguments make sure the transform and Jacobian used for FMA
  # sampling receive the same extra arguments supplied through ameras().
  fits <- list(
    make_fma_summary(c(0, 1), AIC = 10, hess = diag(c(4, 9))),
    make_fma_summary(c(1, 2), AIC = 10, hess = diag(c(4, 9)))
  )
  parnames <- c("alpha", "dose")
